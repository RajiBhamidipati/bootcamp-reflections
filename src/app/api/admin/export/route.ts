...
export async function GET(request: NextRequest) {
  try {
    ...
    const exportData = reflections?.map(reflection => ({
      id: reflection.id,
      user_name: (reflection as any).users?.name || 'Unknown',
      user_email: (reflection as any).users?.email || 'Unknown',
      type: reflection.type,
      title: reflection.title,
      mood_score: reflection.mood_score,
      sentiment_score: reflection.sentiment_score,
      keywords: reflection.keywords?.join(', ') || '',
      overall_mood: reflection.content?.overall_mood || '',
      energy_level: reflection.content?.energy_level || '',
      stress_level: reflection.content?.stress_level || '',
      motivation: reflection.content?.motivation || '',
      daily_highlight: reflection.content?.daily_highlight || '',
      challenges_faced: reflection.content?.challenges_faced || '',
      learning_progress: reflection.content?.learning_progress || '',
      goals_tomorrow: reflection.content?.goals_tomorrow || '',
      gratitude: reflection.content?.gratitude || '',
